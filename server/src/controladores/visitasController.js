...
export class VisitasController {
  ...
  static async listar(url = null, tipo = null) {
    if (tipo === 'clic') {
      ...
      return clics.map(c => new Clic(
        c.id, c.uid, c.url, c.dominio,
        c.elemento, c.posicion_x, c.posicion_y,
    ...
    if (tipo === 'scroll') {
      ...
      return scrolls.map(s => new Scroll(
        s.id, s.uid, s.url, s.dominio,
        s.scroll_x, s.scroll_y, s.porcentaje_scroll,
  ...
  static async crear(data) {
    ...
    if (visitaExistente) {
      if (!visitaExistente.esReciente) {
        ...
        return new Visita(
          visitaActualizada.id, visitaActualizada.uid, visitaActualizada.recurrente,
          visitaActualizada.title, visitaActualizada.url, visitaActualizada.dominio,
          visitaActualizada.userAgent, visitaActualizada.referrer,
          visitaActualizada.ultimavisita, visitaActualizada.created_at
        );
      } else {
        console.log(`Visita reciente (${visitaExistente.segundosTranscurridos}s). No se registra.`);
        const visita = await getVisitaPorUidYUrl(uid, url);
        return new Visita(
          visita.id, visita.uid, visita.recurrente,
          visita.title, visita.url, visita.dominio,
          visita.userAgent, visita.referrer,
          visita.ultimavisita, visita.created_at
        );
      }
    } else {
      console.log("Creando nueva visita...");
      const nuevaVisita = await createVisita({
        uid, recurrente: recurrente || false,
        title, url, dominio, userAgent, referrer
      });
      return new Visita(
        nuevaVisita.id, nuevaVisita.uid, nuevaVisita.recurrente,
        nuevaVisita.title, nuevaVisita.url, nuevaVisita.dominio,
        nuevaVisita.userAgent, nuevaVisita.referrer,
        nuevaVisita.ultimavisita, nuevaVisita.created_at
      );
    }
  }
  ...
  static async registrarClic(data) {
    ...
    if (!uid || !url) throw new Error("UID y URL son campos obligatorios para un clic");

    console.log(`Registrando clic de UID: ${uid} en ${url} — elemento: ${elemento}`);
    ...
    return new Clic(
      nuevoClic.id, nuevoClic.uid, nuevoClic.url, nuevoClic.dominio,
      nuevoClic.elemento, nuevoClic.posicion_x, nuevoClic.posicion_y,
      nuevoClic.timestamp, nuevoClic.created_at
    );
  }
  ...
  static async registrarScroll(data) {
    ...
    if (!uid || !url) throw new Error("UID y URL son campos obligatorios para un scroll");

    console.log(`Registrando scroll de UID: ${uid} en ${url} — ${porcentaje_scroll}%`);
    ...
    return new Scroll(
      nuevoScroll.id, nuevoScroll.uid, nuevoScroll.url, nuevoScroll.dominio,
      nuevoScroll.scroll_x, nuevoScroll.scroll_y, nuevoScroll.porcentaje_scroll,
      nuevoScroll.timestamp, nuevoScroll.created_at
    );
  }
